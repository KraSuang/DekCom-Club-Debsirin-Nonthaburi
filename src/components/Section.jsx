    ...
    return(
        <div className="flex relative rounded-2xl bg-black w-[300px] h-[300px] transition-all duration-300 group hover:scale-[1.05]">
            ...
            <div className="absolute w-full h-full flex flex-col items-center justify-center px-4 py-10">
                <p className={`text-white w-full text-center transition-all lg:duration-200 xl:duration-300 ease-in-out ${isActive('en') ? 'font-[500] text-[30px] xl:group-hover:text-[24px]' : 'text-[30px] font-[500] group-hover:text-[18px]'}`}>{title}</p>
                <p className={`text-white transition-all duration-300 opacity-0 w-full h-0 group-hover:h-full group-hover:opacity-100 ${isActive('en') ? 'font-medium text-lg' : 'font-regular text-lg'}`}>{description}</p>
            </div>
        </div>
    )
}
